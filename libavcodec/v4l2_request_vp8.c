					 ...
					 uint32_t size)
{
    ...
    hdr->flags |= s->invisible ? 0 : V4L2_VP8_FRAME_HDR_FLAG_SHOW_FRAME;
    hdr->flags |= s->mbskip_enabled ? V4L2_VP8_FRAME_HDR_FLAG_MB_NO_SKIP_COEFF : 0;
    hdr->flags |= (s->profile & 0x4) ? V4L2_VP8_FRAME_HDR_FLAG_EXPERIMENTAL : 0;
    hdr->segment_header.flags |= s->segmentation.enabled ? V4L2_VP8_SEGMNT_HDR_FLAG_ENABLED : 0;
    hdr->segment_header.flags |= s->segmentation.update_map ? V4L2_VP8_SEGMNT_HDR_FLAG_UPDATE_MAP : 0;
    hdr->segment_header.flags |= s->segmentation.update_feature_data ? V4L2_VP8_SEGMNT_HDR_FLAG_UPDATE_FEATURE_DATA : 0;
    hdr->segment_header.segment_feature_mode = s->segmentation.absolute_vals;
    ...
    hdr->lf_header.type = s->filter.simple;
    hdr->lf_header.level = s->filter.level;
    hdr->lf_header.sharpness_level = s->filter.sharpness;
    hdr->lf_header.flags |= s->lf_delta.enabled ? V4L2_VP8_LF_HDR_ADJ_ENABLE : 0;
    hdr->lf_header.flags |= s->lf_delta.update ? V4L2_VP8_LF_HDR_DELTA_UPDATE : 0;
    for (i = 0; i < 4; i++) {
        hdr->lf_header.ref_frm_delta_magnitude[i] = s->lf_delta.ref[i];
        hdr->lf_header.mb_mode_delta_magnitude[i] = s->lf_delta.mode[i + MODE_I4x4];
    }
